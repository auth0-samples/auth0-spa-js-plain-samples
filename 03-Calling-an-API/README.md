...
## Tutorial: Calling APIs with Access Tokens

In this tutorial, you will follow on from [the tutorial in part 1](https://github.com/auth0-quickstarts/auth0_new_sdk-spa_quickstart/blob/master/01-login/README.md) to add the ability to call a backend API using an access token.

To support this scenario, you will modify the [ExpressJS](https://expressjs.com) that you created in part 1 so that it supports a new endpoint. This endpoint will require a valid access token to be sent in the `Authorization` header for the call to be successful.

You will then modify the client-side application to retrieve a token from the Auth0 client library, and make a call to the API with this token.
...
### Adding an API to Auth0

For this to work, an API must be added to your Auth0 tenant in order to get an "audience" value to validate the JWT with.

Browse to the [APIs panel](https://manage.auth0.com/dashboard/eu/elkdanger/apis#) in your [Auth0 dashboard](https://manage.auth0.com/dashboard/), and click **Create API**. Give it a name of "Vanilla JS SPA", an identifier value of "https://vanilla-js-spa", and leave the **Signing Algorithm** field as "RS256".

![Creating an API using the Auth0 dashboard](https://cdn.auth0.com/blog/quickstarts:create-api.png)
...
At the end, your `server.js` file should look something like the following:
...
![The demo application showing the response from the server](https://cdn.auth0.com/blog/quickstarts:app-2.png)
